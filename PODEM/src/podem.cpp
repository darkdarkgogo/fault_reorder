...
ATPG::wptr ATPG::fault_evaluate(const fptr fault)
{
	...
	{																	// if fault is on GUT gate output
		...
		if (w->value == U)
			return (nullptr);
		if (fault->fault_type == STUCK0 && w->value == 1)
			w->value = D; // D means 1/0
		if (fault->fault_type == STUCK1 && w->value == 0)
			w->value = D_bar; // D_bar 0/1
		return (w);
	}
	else
	{ // if fault is GUT gate input
		...
		else
		{
			...
			if (fault->node->owire.front()->is_changed())
			{
				...
				return (nullptr); // faulty gate output does not change
		}
	}
	...
	if (fault->node->type != OUTPUT)
	{
		...
		{ // if faulty gate output is U
			...
			if (fault->logical_xor_input)
			{
				...
				{
					/* Either XOR output value exposes an activated input fault. */
					object_level = 0;
...
ATPG::nptr ATPG::find_propagate_gate(const int &level)
{
	...
	for (i = sort_wlist.size() - 1; i >= 0; i--)
	{
		...
		if ((sort_wlist[i]->value == U) &&
				(sort_wlist[i]->inode.front()->is_marked()))
		{ // Check if marked
